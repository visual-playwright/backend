# 🧪 Direct Prompting — Visual Assessment (R-04: Delete)
...
**Skenario file ini: `R-04` — Delete (fixed, tidak perlu isi variabel Skenario ID)**

---

## 📚 INSTRUKSI KONKRET — R-04
...
1. Jalankan 1x langkah Create (pola R-02: cari widget Tambah → isi form data valid → submit) untuk memastikan ada ≥1 baris data

### Skenario Utama: R-04 — Delete (Hapus Data)
1. (Event) Cari widget Delete/Hapus pada baris data target → analisis visual
2. (Action) Klik widget Delete → `playwright_browser_click`
3. (Event) Jika muncul dialog konfirmasi, identifikasi widget konfirmasi → `playwright_browser_take_screenshot`
4. (Action) Klik widget Konfirmasi Hapus → `playwright_browser_click` atau `playwright_browser_handle_dialog`
5. (Event) Verifikasi data tidak lagi tampil di daftar → `playwright_browser_take_screenshot`
...
Nama file: `result_{{PROJECT_ID}}_R-04_{{MODEL_NAME}}_direct.html`
...
<tr><th>Skenario ID</th><td>R-04</td></tr>
