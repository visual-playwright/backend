# 🧪 Direct Prompting — Visual Assessment (R-01: Login)
...
**Skenario file ini: `R-01` — Login (fixed, tidak perlu isi variabel Skenario ID)**

---

## 📚 INSTRUKSI KONKRET — R-01

### Skenario Utama: R-01 — Login
1. (Event) Buka {{URL}}, ambil screenshot → `playwright_browser_navigate`(url={{URL}}), lalu `playwright_browser_take_screenshot`
2. (Event) Identifikasi kolom Username/Email → analisis visual
3. (Action) Klik kolom Username/Email, ketik `{{USERNAME}}` → `playwright_browser_click`, `playwright_browser_type`
4. (Event) Identifikasi kolom Password → analisis visual
5. (Action) Klik kolom Password, ketik `{{PASSWORD}}` → `playwright_browser_click`, `playwright_browser_type`
6. (Event) Cari widget Login/Masuk/Sign In → analisis visual
7. (Action) Klik widget Login → `playwright_browser_click`
8. (Event) Verifikasi indikator autentikasi berhasil → `playwright_browser_take_screenshot`
...
Nama file: `result_{{PROJECT_ID}}_R-01_{{MODEL_NAME}}_direct.html`
...
<tr><th>Skenario ID</th><td>R-01</td></tr>
